...
回退工作区改动时，须先确认**只影响 Agent 自己刚引入的部分**，不波及工作区原有的用户改动。`git checkout -- <file>` 是文件级操作，会丢弃该文件**全部**未提交内容——用于“回退局部某行”时属越权。如需精准回退，用 patch 模式（`git checkout -p`）或手动编辑，只移除 Agent 刚加的那部分。

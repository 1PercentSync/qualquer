...
**语言**：description 用英文（type/scope 已是英文，保持一致；commit message 属面向协作者的正式记录）
...
- **不写会变化的措辞**：所有代码注释/文档禁止使用开发阶段（Phase/Step/Milestone）、未来时态（will / later / for now / 以后 / 待定）等会随开发过时的表述；只描述不会变化的内容
...
工作区出现的 include 删除是 IDE 基于传递性包含分析的结果（已包含的头间接提供了需要的符号）。**在出现编译/链接失败之前不主动恢复被删的 include**——臆断“显式依赖更卫生”不构成恢复依据，编译能通过即说明传递路径当前成立。

### 格式化与 tidy 是用户意图

工作区中的格式化（空格、换行重排）和代码 tidy 应用（删除冗余 include、重排等）均为**用户主动操作**（保存、触发的 reformat、运行 tidy 等）的产物，**代表用户意图，不视为“自动污染”**。提交时一并接受，不逐个质疑或清理。

### 局部回退须限定范围

回退工作区改动时，须将范围限定在**真正想撤的那部分**。`git checkout -- <file>` 是文件级操作，会无差别丢弃该文件**全部**未提交内容——无论是 Agent 刚写的实质改动、用户主动产生的 IDE 格式/tidy、还是上轮累积的待提交内容，一律清空。用于“回退局部某行”时属越权。如需精准回退，用 patch 模式（`git checkout -p`）或手动编辑，只移除真正要撤的那几行。
...
- 句柄缓存开销本可忽略（多为指针级句柄），这不是性能取舍而是设计偏好——既然开销不是理由，就按所有权清晰来设计

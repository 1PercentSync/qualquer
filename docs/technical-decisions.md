...
**决策**：fastgltf（vcpkg 管理）。

**理由**：
- C++17 header-friendly，类型安全，性能好
- Himalaya 使用同一库，加载逻辑可直接照搬
- vcpkg 有端口，集成简单

### Vertex 格式

**决策**：统一 4 属性（position / normal / uv0 / tangent），48 字节。

**理由**：
- 从 Himalaya 的 5 属性精简，去掉 uv1（TEXCOORD_1，用于 lightmap，本项目不需要）
- tangent 保留（normal mapping 必需，MikkTSpace 生成缺失数据）
- host 和 device 共享同一定义

## 纹理

### CUDA BC 压缩纹理

**决策**：使用 CUDA 原生 BC 格式（BC7 + BC5），不用未压缩 RGBA8。

CUDA 11.5 起原生支持 BC1–BC7。`cudaMallocMipmappedArray` 创建 BC 格式 array，`cudaCreateTextureObject` 创建纹理对象（`pResViewDesc` 传 `nullptr`），设备端 `tex2D<float4>()` 采样时硬件自动解压。

**理由**：
- 显存占用降低 4 倍（BC7 = 1 byte/pixel vs RGBA8 = 4 bytes/pixel）
- Himalaya 的 BC 压缩管线（bc7enc + rgbcx + KTX2 缓存）可直接复用
- 项目使用 CUDA 13.2，远高于 11.5 最低要求

**格式选择**（与 Himalaya 一致）：

| TextureRole | BC 格式 | 用途 |
|-------------|---------|------|
...
**决策**：CPU 侧 BC 压缩结果以 KTX2 格式缓存到磁盘（`%TEMP%\qualquer\textures\`），content hash（XXH3_128）为键。

**理由**：
- BC7 压缩耗时（尤其高质量模式），同一纹理重复加载时直接读缓存跳过压缩
- KTX2 是 Khronos 标准容器格式，自描述 mip chain
- 从 Himalaya 照搬缓存 + KTX2 模块

## 材质

### GPU 材质数据布局

**决策**：`GPUMaterialData` 80 字节，`alignas(16)`，从 Himalaya 直接照搬。

纹理索引字段（`base_color_tex` 等）存储 `cudaTextureObject_t` 数组的索引。场景加载时收集所有纹理 object 到 `CudaBuffer<cudaTextureObject_t>`，上传到设备，shader 通过索引取得 handle 再采样。

**理由**：
- CUDA 无 Vulkan 的 bindless descriptor array，但 texture object 数组 + 索引是等价方案
- 材质结构体 host/device 共享，CUDA 天然对齐兼容

### Default Textures

**决策**：三个 1×1 R8G8B8A8_UNORM 纹理（white、flat_normal、black），缺失纹理槽用对应 default 填充。

**理由**：
- shader 无条件采样，无需 if 判断纹理是否存在
- 1×1 纹理不做 BC 压缩（极值处 sRGB/linear 结果一致，且 BC block 最小 4×4）
- 与 Himalaya 一致

### Alpha 处理

**决策**：`GPUMaterialData` 保留完整 alpha_mode 字段，渲染端全部视为 opaque。

**理由**：
- 材质数据完整，后续 Phase 接入 alpha mask / stochastic alpha 时无需改数据层
- BLAS 构建不设 `OPTIX_GEOMETRY_FLAG_DISABLE_ANYHIT`，但也不注册 anyhit shader，等效 opaque 行为
- 保留未来 alpha 接入空间，无额外开销

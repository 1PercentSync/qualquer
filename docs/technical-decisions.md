...
| 显示 buffer | 1 | B8G8R8A8_UNORM | Vulkan 分配，CUDA 导入 | LDR 输出，Vulkan blit 到 swapchain |
...
- 显示 buffer 是 LDR（B8G8R8A8），带宽最小
- 单显示 buffer 在 T_累积 > T_呈现 时足够（典型 PT 场景）

### 显示 Buffer 细节

**格式**：`VK_FORMAT_B8G8R8A8_UNORM`

与 swapchain（`B8G8R8A8_SRGB`）通道顺序一致，blit 只需线性→sRGB 编码，无需通道重排。CUDA kernel 按 BGRA 字节顺序写入。

...
| pInSpecularHitDistance | R32F | bounce 1 specular ray 的 `optixGetRayTmax()` |
...
**Specular Motion**：提供 pInSpecularHitDistance + 两个矩阵，SDK 内部推导 specular motion vectors（比直接计算更简单，hit distance 在 bounce 1 的 optixGetRayTmax 即可获得）。

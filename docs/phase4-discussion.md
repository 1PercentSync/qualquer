...
**背景**：Himalaya 未实现能量补偿（`shaders/common/brdf.glsl` 仅含标准 D_GGX + V_SmithGGX + F_Schlick）。GGX 在 roughness=1、F=1 时能量损失约 60%。
...
> 基于 D1-D28 决策结果划分 Phase 4 与 Phase 4.5 的特性边界。
...
- BRDF 全集（D_GGX, V_SmithGGX, F_Schlick, EON diffuse + CLTC 采样, VNDF, multi-lobe selection, combined PDF）+ 多散射能量补偿（specular: Turquin + Sforza 多项式, diffuse: EON 内建）— D8, D27, D28a

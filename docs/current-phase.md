...
- 编译标志：`--optix-ir --gpu-architecture=compute_89 -std=c++20 -Xcompiler=/utf-8 -I${OptiX_INCLUDE}`。依据（CUDA 13.2 nvcc 文档）：`--gpu-architecture` 正式情况下必须是 virtual 架构（compute_89），OptiX IR 需 virtual 供运行时 JIT；`-std=c++20` 显式指定，因 nvcc 默认 C++ dialect 跟随 host compiler 不确定；`-Xcompiler=/utf-8` 抑制 MSVC 代码页 936 下的 C4819（与顶层 add_compile_options 同源，自定义命令绕过全局处理）；`--machine=64`/`--ccbin` 经查官方默认已满足，不加
...
`optix::Pipeline` 类：持有 OptixModule + ProgramGroup[] + OptixPipeline handle。init 接收 OptixDeviceContext + .optixir 文件路径 + launch params 的 size 和变量名，内部读文件创建 Module。

- **ProgramGroup** 作为公开命名成员暴露（raygen / miss / hitgroup）——renderer 需要它们来 pack SBT records
- **init 额外接收 launch params 的 size 和 device 侧变量名**：optix 层不能 include renderer 的 launch_params.h（编译期单向依赖硬约束），故 `sizeof(LaunchParams)` 与 `__constant__` 变量名无法在 optix 层取得，由 renderer 传入
- **PipelineCompileOptions**（取值依据 `optix_types.h` 注释）：
  - `numPayloadValues=0`：合法范围 [0..32]，Phase 2 不发射光线、不用 payload，设 0 精确描述当前；后续 Phase 用 payload 时调大
  - `numAttributeValues=2`：合法范围 [2..8]，最小 2 即三角形重心坐标
  - `traversableGraphFlags=ALLOW_SINGLE_LEVEL_INSTANCING`：为 Phase 3 的 TLAS→BLAS 铺路；Phase 2 traversable=0 不遍历，不受影响
  - `pipelineLaunchParamsSizeInBytes=sizeof(LaunchParams)`（9.1 可选，下个大版本强制，现在就设好）
...
display_stream:  wait raygen_done[1-slot] + wait reverse_sem → [tonemap ← buf[A]] → record tonemap_done[slot] → signal forward_sem
Vulkan queue:    wait forward_sem → [blit] → signal reverse_sem → [ImGui] → present
```

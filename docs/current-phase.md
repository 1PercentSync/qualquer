...
1. `brdf.cuh` 新增 `brdf_pdf(BrdfParams, L) → float`，与 `brdf_eval_and_pdf` / `brdf_sample` 形成三件套。消除 NEE/MIS PDF 组装重复（与 `combined_lobe_pdf` 独立函数的设计理由一致——多处调用同一函数保证 MIS 一致性）。
...
**累积 reset 检测重分类**：当前 `settings_changed` 包含 `max_bounces` 和 `samples_per_frame`，每次变化都 reset 累积。Step 15 自适应 spp/bounces 会每帧改这些值——如果每帧 reset，Separate Sum 退化为 1spp 闪烁。`samples_per_frame` 是纯吞吐旋钮（Separate Sum 的 per-sample 分布不变，count 正确即可），**现在就不应该 reset**。`max_bounces` 有 bias（硬截断丢能量），但混合不同 max_bounces 的 sample 在交互预览下可接受，自适应更是每帧变化。

从检测中移除 `max_bounces` / `samples_per_frame`，删除 `prev_max_bounces_` / `prev_samples_per_frame_`。剩余检测对象按语义分组：camera（`inv_view` + `inv_projection`）打包为 `CameraKey` 结构体（C++20 defaulted `operator==`，消除 padding 风险）；`env_rotation` 和 `dlss_enabled` 保持独立 `prev_` 成员（只 2 个字段，结构体化收益不大）。`prev_dlss_preset_` 继续独立（触发 feature recreate 而非 reset）。7 个 prev_ → 4 个（CameraKey + 2 标量 + preset）。

暂停期间的 invalidation 保持区分：camera 变化 → `invalidate_dlss_history()`（视角跳变，旧 slot 的 depth/MV/normals 和新视角不匹配）；content 变化 → 仅设 `reset_requested_`（env_rotation 是连续光照变化，DLSS 通过时域滤波适应；dlss_enabled 切换通过 feature 生命周期路径单独处理 invalidation）。
...
**`env_rotation_sin/cos` 移入 `EnvLightData`**：rotation 只和 env 相关——miss shader 用它旋转 cubemap lookup 方向，NEE 用它旋转采样/PDF 方向，都是"环境光在世界空间中的朝向"。移入后 miss shader 和 NEE 统一从 `params.env` 读取，LaunchParams 减少两个散装字段。

**device 侧 env helper 签名打包**：host 侧 `EnvLightData` 打包后，device 侧 `env_pdf` 和 `sample_env_alias_table` 仍接收散装参数，调用点（`evaluate_env_nee`、closesthit env MIS weight）从 `params.env` 逐字段拆开传入——同一原则的不完整应用。两个函数改为接收 `const EnvLightData&` 替代散装参数。`emissive_light_pdf` 不动——它只取 `total_power` 一个字段，传整个 `EmissiveLightData` 引入不需要的字段。

...
> 讨论记录见 `docs/phase4-discussion.md`（D1-D41 决策）。
...
Step 1 包含 Pipeline 变更（numPayloadValues=16），后续 Step 均依赖此配置。
...
所有决策详见 `docs/phase4-discussion.md`（D1-D41）。
...
| D2 Payload | 全信息 16 registers | SER 重排后 payload 随线程移动，零全局内存 |
...
| p9-p11 | color (float3) — emissive + NEE radiance（miss shader 已含 env MIS 权重） |
...
| p14 | sample_index (uint32) — raygen 传递给 closesthit 用于 RNG |
| p15 | bounce index (uint32) — raygen → closesthit 单向传递，closesthit 不写回 |
...
Primary ray（bounce 0）跳过 SER，直接 `optixInvoke`——primary ray 天然空间 coherent（相邻像素命中相邻几何体和相似材质），reorder 按材质重组反而打散空间局部性，伤害纹理 cache 和 aux surface-write coalescing。

Secondary ray（bounce ≥ 1）经 `optixTraverse` 后查询 hit object，取 `material_buffer_offset & 0x3FF` 作为 hint（**10 bits**，掩码）调用 `optixReorder`。BRDF 采样后方向随机、空间局部性已被破坏，材质聚合是净收益。miss 时 hint=0，自然与所有 hit 材质分离。代价：每线程每 secondary bounce 一次 `geometry_infos[]` global memory 读取。
...
| **10-bit 掩码 hint** | 680 | 1100 |
| 12-bit 掩码 hint | 680 | 1090 |
| 16-bit 裸传 hint | 676 | 1062 |

上表为所有 bounce 统一 reorder 的历史数据。当前实现 primary ray 跳过 reorder，预期 1spp 吞吐恢复至接近 SER 关的水平（消除 primary bounce 的 reorder 开销），同时 secondary bounce 保留 SER 收益。

**结论与选择理由**：

- SER 本体在高 spp 下收益显著（+23% vs 无 reorder），必须保留
- Primary ray 跳过 reorder：相邻像素的空间局部性是 primary ray 的主要 cache 优势，SER 重组会破坏它
- 排序成本随 hint 位数平滑递增（每 4 bits 约 -2~4%）。原 16-bit 裸传声明了 10 个恒零高位，纯浪费排序预算，属用法错误
- 材质 hint 的分组收益在 warehouse 默认视角测不出（8-bit vs 零参 +0.6%，噪声内）——该视角材质空间聚集度高、材质种类单一，不代表所有视角和场景；材质发散的视角或场景中 hint 的分组收益预期更显著
...
uint32_t dlss_enabled;  // 1 = 单帧输出（无累积），0 = Separate Sum
float jitter_x, jitter_y;  // DLSS ON 时全局 per-frame jitter

// 场景光源资源（POD 结构体打包）
EnvLightData env;           // cubemap + alias table + rotation sin/cos
EmissiveLightData emissive; // triangles + alias table + total_power

// Aux G-buffer surfaces（closesthit/raygen surf2Dwrite 写入）
cudaSurfaceObject_t aux_depth, aux_motion_vectors, aux_diffuse_albedo,
                    aux_specular_albedo, aux_normals, aux_roughness;

// MV 矩阵
float4x4 view_projection;       // 当前帧 unjittered VP
float4x4 prev_view_projection;  // 前帧 unjittered VP

// Sobol RNG
uint32_t sobol_directions[4096]; // __constant__ memory，128 维 × 32 bit
```
...
| frame_slots_[i].sample_count | 每个 ping-pong FrameSlot 的真实 sample 数 | reset 不直接改——chain_count 归零使 raygen 覆写，write slot 的 count 在帧末设为 1 |
...
>
> 早期实现曾用平行数组 `accum_counts_[2]`；Step 14.5 将其并入 `FrameSlot::sample_count`，与 color/aux/metadata 同槽。

### Separate Sum 写读配对（per-slot 计数）

每个 ping-pong FrameSlot 自带 sample 数（`frame_slots_[i].sample_count`），sum 和 count 作为配对数据一起走，取消全局 `sample_count_` 和 buffer 清零。

- **写（raygen）**：host 算 `chain_count = camera_changed ? 0 : frame_slots_[read_slot].sample_count`，上传为 `LaunchParams::sample_count`。raygen 据此决定覆写（= 0）还是累加（> 0）。帧末 `frame_slots_[write_slot].sample_count = chain_count + 1`。
- **读（tonemap）**：直接用 `frame_slots_[read_slot].sample_count` 作除数（buffer 内容与 count 永远配对）。count == 0（init/resize 后首帧）时 tonemap 输出黑——此时 buffer 未初始化，raygen 正在写另一个 buffer 的第一个 sample。
...
struct EmissiveTriangle {
    float3 v0;       float normal_x;      // 16B — 预计算法线 X
    float3 edge1;    float normal_y;      // 16B — 预计算边 v1-v0 + 法线 Y
    float3 edge2;    float normal_z;      // 16B — 预计算边 v2-v0 + 法线 Z
    float3 emission; uint32_t emissive_tex; // 16B — emissive_factor + 纹理索引
    float2 uv0;      float2 uv1;           // 16B
    float2 uv2;      uint32_t double_sided; uint32_t _pad; // 16B
};
```

96 字节不变。预存 edge vectors、归一化法线、emissive 纹理索引和 double_sided，device 端消除每次 NEE 采样的 cross+normalize 和随机 Material 数组读取。host 构建时从顶点计算并填入。
...
- `BrdfParams`：着色点不变量打包（V_ts/N/T/B、F0=lerp(0.04,base_color,metallic)、base_color、alpha、r、NdotV、turquin_comp(RGB)、E_glossy_rgb(RGB)、diffuse_weight=(1-metallic)×(1-E_glossy)(RGB)、p_spec）。CLTC invariants（LTC 矩阵、混合权重）不在 BrdfParams 中，由 `CltcInvariants` 结构体在 `brdf_eval_and_pdf` / `brdf_sample` 内部按需计算——避免跨 shadow trace 的栈压力
- `init_brdf_params(...)`：构造入口，集中 D27/D28c 逐通道补偿（turquin_comp 逐通道调 turquin_compensation 共用标量 E_ss；E_glossy 逐通道用混合 F0，标量快速路径跳过 dielectric 冗余计算；diffuse_weight 双重衰减）；纯金属（metallic=1）跳过 E_glossy、diffuse_weight=0
- `brdf_eval_and_pdf(params, L, NdotL)`：返回 specular（D·V·F·turquin_comp）+ diffuse（diffuse_weight·f_EON）BRDF 值与 combined PDF（融合计算共享 L_ts/H_ts）
...
// dim 0-1 (jitter): sequence_index = frame_index（D37 per-frame jitter）
// dim 2+  (BRDF/NEE): sequence_index = sample_index
__forceinline__ __device__ float sobol_rng(const uint32_t *directions,
                                           uint32_t pixel_index, uint32_t sequence_index,
                                           uint32_t frame_index, uint32_t dimension) {
    if (dimension >= 128) {
        uint32_t h = xxhash32(pixel_index, sequence_index, dimension);
        return static_cast<float>(h >> 8) * 0x1p-24f;  // [0, 1)
    }
    uint32_t sobol_val = sobol_sample(directions, dimension, sequence_index);  // __ffs() bit scan
    uint32_t pixel_offset = pcg_hash(pixel_index * 0x1f1f1f1fu ^ dimension);
    uint32_t temporal_offset = frame_index * 2654435769u;  // golden-ratio
    sobol_val += pixel_offset + temporal_offset;
    return static_cast<float>(sobol_val) / static_cast<float>(0xFFFFFFFFu);
}
// 调用方：
//   jitter:    sobol_rng(dirs, pixel, frame_index, frame_index, 0/1)  — per-frame
//   BRDF/NEE:  sobol_rng(dirs, pixel, sample_index, frame_index, dim) — per-sample
```
...
**DLSS ON color 均值**：D32 规定 color 输入为"同一亚像素位置 N 个 sample 的辐射度平均"。`effective_spp > 0` 时，raygen 将 sample loop 的 `frame_radiance` 累加总和除以 `effective_spp` 后写入 color write slot，并将该 slot 的 `FrameSlot::sample_count` 置 1；该值表示已经归一化的一份单帧 radiance，切回 DLSS OFF 时不能再次按 spp 除法。`effective_spp == 0` 时不生成 color/aux/metadata、不改变 slot validity、不翻转 ping-pong，也不重复 evaluate 同一 input；已有 DLSS output 有效时直接 tonemap 最后输出，否则 fallback tonemap 当前 read color，从而保持暂停画面不变。暂停期间若相机或投影变化，视为时域断点——调用 `invalidate_dlss_history()` 使 input slots 失效并请求 reset，但保留 `dlss_output_valid_` 使冻结画面不闪；恢复后首帧以 `InReset=1` 和零 MV 开启新历史。
...
Step 11–14 实现过程中 Renderer 膨胀到 38 个成员、submit_cuda 超过 430 行、closesthit 超过 400 行。以下重构在正确性修复之后、Step 15 之前完成，防止后续特性（自适应 spp、Ray Cone、M2 ReSTIR）进一步恶化结构。
...
**closesthit 瘦身**（已完成）：NEE 组装逻辑原占 closesthit 约 40% 行数（~140/400），与交点处理正交。已完成三个子步骤：

1. `brdf.cuh` 新增 `brdf_pdf(BrdfParams, L) → float`，与 `brdf_eval_and_pdf` / `brdf_sample` 形成三件套，消除 NEE/MIS PDF 组装重复。
2. `nee.cuh` 新增 `evaluate_env_nee` / `evaluate_emissive_nee` 两个独立内联函数，各自封装从 alias table 采样到 MIS weight 的全流程；closesthit NEE 缩为两行调用 + 一行累加。
3. device helper `write_aux_no_surface(sx, sy, float4 diffuse_albedo)` 参数化 diffuse_albedo，消除 pass-through 与 raygen sky 的 aux 默认值写入重复。

**累积 reset 检测重分类**（已完成）：从检测中移除 `max_bounces` / `samples_per_frame`（质量/吞吐旋钮，不改变积分对象；Step 15 自适应会每帧变化）。剩余检测对象：camera（`inv_view` + `inv_projection`）打包为 `CameraKey`；`env_rotation` 和 `dlss_enabled` 保持独立 `prev_` 成员；`prev_dlss_preset_` 继续独立（触发 feature recreate 而非 reset）。

暂停期间的 invalidation 保持区分：camera 变化 → `invalidate_dlss_history()`；content 变化 → 仅设 `reset_requested_`（env_rotation 是连续光照变化；dlss_enabled 切换通过 feature 生命周期路径单独处理）。

**SceneRenderInput 场景资源打包**（已完成）：在 `launch_params.h` 中定义 `EnvLightData` / `EmissiveLightData` POD 结构体，SceneLoader 返回、SceneRenderInput 持有、LaunchParams 内嵌，替代散装字段三处镜像。

**`env_rotation_sin/cos` 移入 `EnvLightData`**（已完成）：miss shader 和 NEE 统一从 `params.env` 读取，LaunchParams 减少两个散装字段。

**device 侧 env helper 签名打包**（已完成）：`env_pdf` 和 `sample_env_alias_table` 接收 `const EnvLightData&`。`emissive_light_pdf` 不动（只取 `total_power` 一个字段）。

**DlssRR 所有权移入 Renderer**（已完成）：`dlss_rr_` 为 Renderer 成员（init 时从 Context 取 device_id）；`dlss_preset` 在 RenderSettings；submit_cuda 不再接收 `DlssRR&`；DebugUIContext 通过 `Renderer::dlss() const` 借用。

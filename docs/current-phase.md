...
KTX2 缓存模块支持实际需要缓存的格式集：BC7 UNORM/SRGB、BC5 UNORM、BC6H UFloat，以及 2D 与 cubemap（faceCount=6）。与 Himalaya 的差异：去掉 R16G16B16A16_SFLOAT（Himalaya 中用于 IBL cubemap 的未压缩中间格式，Qualquer 的 BC6H 由 ISPCTextureCompressor CPU 端直接产出，fp16 数据不经 KTX2 缓存）；去掉 B10G11R11_UFLOAT_PACK32 和 R16G16_UNORM（Himalaya 的 irradiance cubemap 和 BRDF LUT 格式，属于 Split-Sum 近似产物，纯 PT 不需要——漫反射和镜面反射积分由蒙特卡洛光线追踪直接求解）。Phase 3 LDR 材质用 BC7/BC5 子集，HDR IBL 纹理用 BC6H。
...
**Material**（host + device 共享）：

从 Himalaya 的 `GPUMaterialData` 直接照搬并改名 `Material`（去掉 `MaterialInstance` 后“GPU/Data”前后缀失去区分意义）。80 字节，`alignas(16)`。CUDA 天然对齐兼容，结构体布局 host/device 一致。
...
**为何去掉 `MaterialInstance`**：Himalaya 的 `MaterialInstance` 与 `GPUMaterialData` 重复存了 `alpha_mode`/`double_sided`，且 `template_id` 全库不读、`buffer_offset` 恒等于 `material_id`、`double_sided` 在 CPU 侧从不读取。Qualquer 让 `Material` 成为唯一材质表示：`material_id` 直接索引 `Material[]`（1:1），`alpha_mode`/`double_sided` 只存这一份。AS builder 收 `Material[]`，读 `alpha_mode`（== 0 判 opaque）设 BLAS flag，`GPUGeometryInfo.material_buffer_offset = material_id`。
...
struct GPUGeometryInfo {
    ...
    uint32_t material_buffer_offset;     // 16: Material 数组索引（= material_id，加载时 1:1）
...
2. **load_materials**：加载 sampler → 加载纹理（hash → cache check → decode → BC compress → CUDA upload）→ 填充 `Material`（含 default texture fallback）→ 上传材质数组到 `CudaBuffer<Material>`
...
新增:
  ...
  third_party/ispc_texcomp/CMakeLists.txt
  third_party/ispc_texcomp/include/qualquer/ispc_texcomp/ispc_texcomp.h
  third_party/ispc_texcomp/src/ispc_texcomp.cpp
  third_party/ispc_texcomp/src/kernel.ispc
  ...
  renderer/include/qualquer/renderer/mesh.h
...
|---------------|----------------|---------|
| Vertex（`vertex.h`） | `framework/mesh.h` | 去 uv1、去 Vulkan 方法；拆为独立纯数据头（device 安全） |
...
| 纹理处理（mip/BC7/BC5/cache） | `framework/texture.h/.cpp` | GPU 上传从 Vulkan 改 CUDA |
| BC6H 压缩 | `framework/texture_compress.h/.cpp` | Vulkan GPU compute → ISPCTextureCompressor CPU ISPC |
| KTX2 缓存 | `framework/ktx2.h/.cpp` | 格式枚举适配 |
| content hash | `framework/cache.h/.cpp` | 缓存路径改 qualquer |
| Material（原 GPUMaterialData） | `framework/material_system.h` | 改名 Material；去 MaterialInstance（重复字段，参考项目 CPU 侧未用） |
| Default textures | `framework/texture.h/.cpp` | CUDA 纹理创建替代 Vulkan |
| SceneLoader | `app/scene_loader.h/.cpp` | buffer/纹理创建改 CUDA；gltf_utils（parse_gltf、transform_aabb）内联进 scene_loader.cpp |
...
| MikkTSpace tangent（`mesh.h` + `mesh.cpp`） | `framework/mesh.cpp` | 直接照搬；声明拆到 `mesh.h`（host-only，include `vertex.h`） |

...
**选项摘要**：
- **A. 全信息 Payload**（16 registers，Phase 4.5 ray cone 扩展至 18）：next_origin, next_direction, throughput_update, color, hit_distance, last_brdf_pdf, sample_index, bounce — 全走 registers。env MIS weight 已移入 miss shader（仅 miss 时需要）
...
**决策**：✅ **A. 全信息 Payload（16 registers）**。SER 重排后 payload 随线程移动、零全局内存访问；B/C 的 global buffer 在 SER 重排后访问变为 non-coalesced，反而更差。
...
2. **Per-slot 计数**：每个 ping-pong slot 各自记录 sample 数，sum 和 count 作为配对数据一起走。tonemap 的除数永远与所读 buffer 的内容匹配。（原方案使用全局 `sample_count_` + 清零两 buffer，实现后发现 reset 帧 clear 与同帧 tonemap 读同一 buffer 存在跨 stream 数据竞争且导致黑帧，按决策可更新性原则改为 per-slot 计数 + 覆写模式。早期实现为平行数组 `accum_counts_[2]`；Step 14.5 并入 `FrameSlot::sample_count`，与 color/aux/metadata 同槽。）
3. **累积重置**：camera/content 参数/渲染分辨率/场景切换时 `chain_count` 归零 → raygen 覆写 write buffer（不读 read buffer），等效清零但无 memset 和跨 stream 竞争。tonemap 同帧仍读 read buffer（count 有效），显示上一帧画面。
4. **语义拆分**：`frame_counter_`（帧号，永不 reset——%2 选 slot，同时上传为 LaunchParams::frame_index 作 device temporal/RNG 源）、`frame_slots_[i].sample_count`（per-slot sample 数，帧末更新 write slot）。不单设 `frame_seed_`：frame_counter_ 本身单调递增、永不 reset，复用作 RNG temporal scramble 源即可。
...
- ~~Cosine hemisphere sampling~~ → CLTC sampling（D28a，EON 配套）
- Combined multi-lobe PDF
- Orthonormal basis construction

**讨论点**：
- 目标是"完整 PT 但不做性能优化"——上述全集是否都算"完整 PT 的基本组件"而非"优化"？
- VNDF sampling 和 importance sampling 属于"让 PT 正确/可用的基本采样"还是"性能优化"？
- 如果不做 importance sampling，uniform hemisphere sampling 是否能在合理时间内收敛？

**决策**：✅ **全做**。上述全集是完整 PT 的最小 BRDF 集，不是优化。CLTC / VNDF / multi-lobe selection 是让 PT 在可行时间内产出可辨识结果的基本采样策略——没有它们 specular 材质实际不可用。Diffuse 模型由 Lambertian 升级为 EON + CLTC 采样（D28a）。
...
**brdf_eval_and_pdf / brdf_sample 接口 + BrdfParams**：multi-lobe BRDF 求值与采样抽成独立接口函数 `brdf_eval_and_pdf`（融合 eval 和 combined PDF，共享 L_ts/H_ts）/ `brdf_sample`，着色点不变量打包为 `BrdfParams` 结构体、由 `init_brdf_params` 构造入口一次性填充，替代 Himalaya 的 closesthit 内联。CLTC invariants 不在 BrdfParams 中，由 `CltcInvariants` 在 eval/sample 内部按需计算（避免跨 shadow trace 栈压力）。理由：与 combined_lobe_pdf 同理，eval/sample 在 NEE eval 两处复用，接口化保证 BRDF 求值一致；`init_brdf_params` 集中 D27/D28c 逐通道补偿计算（turquin_comp 共用标量 E_ss、E_glossy 标量快速路径跳过 dielectric 冗余、diffuse_weight 双重衰减），避免散到 closesthit；`__forceinline__` 零运行时开销。VNDF 采样产生下半球方向（L_ts.z ≤ 0）时 brdf_sample 返回 pdf=0/throughput_update=0 表示无效，path 终止逻辑由 closesthit 处理。
...
**决策**：✅ **纳入**（早期）。`sample_count >= target_sample_count` 时跳过 launch。UI 暴露 `target_sample_count`（0 = 无限累积）。

> **已被 D35 取代**：DLSS-RR 内部管理时域历史后，target sample count 无有意义的收敛停止点，特性移除。
...
**早期拟定的 Phase 4 运行时参数（后续被 Phase 边界 / D29 / D35 修订）**：
- max_bounces（int, 默认 16）— 保留
- samples_per_frame（int, 1~64）— 保留
- ~~render_scale（float, 0.25~1.0）~~ → Phase 4.5 改为 `render_height` 滑块（D15 机制修正）
- exposure（float，实现为 exposure_ev）— 保留
- ~~ibl_rotation~~ → Phase 4.5 Step 9（D22）
- ~~target_sample_count~~ → 被 D35 移除
- ~~debug view 切换~~（改为临时代码验证，不做持久 UI 参数）

**决策**：✅ **以 Phase 边界 + D29 审视 + D30 架构为准**。Phase 4 Step 8 实际暴露：max_bounces、samples_per_frame、exposure_ev、FOV、accumulation_enabled、Reset、场景统计。无 indirect_intensity（D24）、无 tonemap 切换（D17 仅 PBR Neutral）、无自适应参数（D16，归 4.5 Step 15）。
...
2. **Payload pack/unpack helpers**：closesthit→raygen 的 16 register 编解码封装在 `payload_helpers.cuh`，改布局只需改一处。
...
2. **`SceneStats`**（renderer 层）——只读场景资产快照：
   - meshes, instances, tlas_instances, materials, textures, triangles, vertices, emissive_triangles, env_map_width, env_map_height
   - Application 在 load_scene 后统计并缓存；UI 文案将 `meshes` 显示为 "Meshes/BLAS"
...
6. **Deferred slider**：`slider_float_deferred` + `slider_angle_deferred` + `slider_uint_on_release` 作为 debug_ui.cpp 匿名命名空间工具函数

---

## Phase 划分

> 基于 D1-D41 决策结果划分 Phase 4 与 Phase 4.5 的特性边界。
...
MUSTREAD:7
- OptiX Denoiser / DLSS-RR（D31 后由 DLSS-RR 落地于 4.5）

### Phase 4.5：收敛质量 + DLSS-RR + 自适应

> D31-D41 基于 Phase 4 完成后的重新评估，更新了部分早期决策。详见末尾 §Phase 4.5 决策。
...
| 8 | Ray Cone LOD | D13 | 纹理 aliasing 降噪，payload 扩展至 18 registers（p16/p17） |
...
- vk_gltf_renderer：`D:\Github\vk_gltf_renderer`（NVIDIA 官方 Vulkan RT glTF 渲染器，DLSS-RR + 完整 PBR metallic-roughness 工作流，diffuse/specular albedo 对照参考）
- DLSS SDK：`D:\Github\DLSS`（`doc/DLSS-RR Integration Guide.pdf`，`include/nvsdk_ngx_helpers_dlssd_cuda.h`）

**弱参考**（未合并 PR，仅供交叉对照）：
- Blender Cycles DLSS：`D:\Github\blender-dlss`（分支 `dlss-pr`，PR #153077，`intern/cycles/integrator/denoiser_dlss.cpp`）

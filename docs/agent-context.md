...
- **进度**：Phase 4.5 Step 14.5 结构重构 7/7 完成（FrameSlot、closesthit 瘦身、累积 reset 重分类、SceneRenderInput 打包、env rotation 移入 EnvLightData、env helper 签名打包、DlssRR 所有权移入 Renderer）；剩余 sobol 外移为 benchmark 项

### 下一个任务

Phase 4.5 Step 14.5 重构：LaunchParams sobol 外移验证（需用户在 CLion 中 benchmark）

...
- [ ] 创建 `renderer/src/tonemap.cu`（读 float4 累积 buffer → clamp → 写 uchar4 到 display surface）并更新 `renderer/CMakeLists.txt`（新增 tonemap.cu）
- [ ] 请求用户在 CLion 中编译验证
...
- [ ] `submit_cuda` 实现更新：LaunchParams 填充 + 上传 → `optixLaunch`（pipeline, stream, params, sbt, w, h, 1）→ tone map → signal semaphore；ping-pong 索引翻转

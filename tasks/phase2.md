...
- [x] `renderer::Renderer::submit_cuda`：拆分双 stream 提交（compute_stream: wait + upload + raygen + record; display_stream: wait + tonemap + record + signal）
- [x] display_surface 反向同步：新增单个反向 interop semaphore（Vulkan signal after blit → CUDA wait before tonemap；单个即够——reverse signal 经 forward per-frame 链强制交替）；end_frame 的 submit signal 列表新增反向 sem（stage=TRANSFER_BIT）；submit_cuda 的 display_stream 新增 wait；init 中 pre-signal 闭合首帧；acquire 失败路径的 drain submit 新增 signal
- [x] 请求用户在 CLion 中编译验证（窗口显示纯色 + ImGui，功能不变，无 validation / OptiX / CUDA 报错）

...
- [ ] display_surface 反向同步：新增单个反向 interop semaphore（Vulkan signal after blit → CUDA wait before tonemap；单个即够——reverse signal 经 forward per-frame 链强制交替）；end_frame 的 submit signal 列表新增反向 sem（stage=TRANSFER_BIT）；submit_cuda 的 display_stream 新增 wait；init 中 pre-signal 闭合首帧；acquire 失败路径的 drain submit 新增 signal

...
- [ ] 重构（草案，执行前需讨论）：`env_rotation_sin/cos` 移入 `EnvLightData`——rotation 只和 env 相关，miss shader 和 NEE 统一从 `params.env` 读取，LaunchParams 减少两个散装字段
- [ ] 重构（草案，执行前需讨论）：device 侧 env helper 签名打包——`env_pdf` 和 `sample_env_alias_table` 改为接收 `const EnvLightData&` 替代散装参数，消除调用点逐字段解包

...
- [ ] 重构：closesthit 瘦身——`brdf.cuh` 新增 `brdf_pdf(BrdfParams, L, NdotL)` 消除 3 处 NEE/MIS PDF 重复；`nee.cuh` 新增 `evaluate_env_nee` / `evaluate_emissive_nee` 两个独立内联函数将 ~140 行 NEE 逻辑提出 closesthit；device helper `write_aux_no_surface(sx, sy, diffuse_albedo)` 参数化 diffuse_albedo 消除 pass-through 与 raygen sky 的 aux 默认值写入重复

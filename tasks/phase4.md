...
- [ ] 重构：累积 reset 检测重分类——从检测中移除 `max_bounces` / `samples_per_frame`（质量/吞吐旋钮，不改变积分对象，自适应会每帧变化）；camera 打包为 `CameraKey`（`inv_view` + `inv_projection`，C++20 defaulted `==`），`env_rotation` / `dlss_enabled` 保持独立 prev_ 成员；暂停期间仅 camera 变化触发 `invalidate_dlss_history`，content 变化不触发

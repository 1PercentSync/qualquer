...
- [ ] BRDF eval + sample 接口：build_orthonormal_basis、specular_probability、BrdfParams 结构体 + init_brdf_params 构造入口（集中 E_glossy/Turquin 逐通道补偿与 diffuse_weight 双重衰减）、brdf_eval（specular+diffuse BRDF 值，Step 7 NEE 用）、brdf_sample（specular VNDF / diffuse CLTC + throughput_update + combined PDF，Step 5 closesthit 用）

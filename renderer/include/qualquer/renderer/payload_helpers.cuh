 ...
 * @brief 18-register OptiX payload pack/unpack helpers (renderer layer).
 */
...
namespace qualquer::renderer {
    ...
    /**
     * @brief All 18 payload registers unpacked into named fields.
     *
     * Raygen constructs this from the 18 local uint32_t variables that
     * optixTrace populated. Changing the register layout only requires
     * updating payload_unpack and the closesthit setters below.
     */
    struct PayloadData {
        ...
        float3 color; ///< Emissive + NEE radiance from this hit.
        float hit_distance; ///< Ray tHit (negative = miss, terminates path).
        float env_mis_weight; ///< MIS weight for BRDF-sampled env hit.
    ...
    /**
     * @brief Unpacks 18 payload registers into PayloadData.
     *
    ...
    __forceinline__ __device__ PayloadData payload_unpack(
        ...
        const uint32_t p15, const uint32_t p16, const uint32_t p17) {
        (void) p15;
        (void) p16;
        PayloadData d{};
        ...
        d.env_mis_weight = __uint_as_float(p13);
        d.last_brdf_pdf = __uint_as_float(p14);
        d.bounce = p17;
        return d;
    }

    // ---- Closesthit setters (write individual payload fields) -------------------
    ...
    /** @brief Writes env_mis_weight into p13. */
    __forceinline__ __device__ void payload_set_env_mis_weight(const float v) {
        optixSetPayload_13(__float_as_uint(v));
    }

    /** @brief Writes last_brdf_pdf into p14. */
    __forceinline__ __device__ void payload_set_last_brdf_pdf(const float v) {
        optixSetPayload_14(__float_as_uint(v));
    }

    /** @brief Writes bounce index into p17. */
    __forceinline__ __device__ void payload_set_bounce(const uint32_t v) {
        optixSetPayload_17(v);
    }

    /** @brief Writes sample_index into p15 (raygen passes to closesthit for RNG). */
    __forceinline__ __device__ void payload_set_sample_index(const uint32_t v) {
        optixSetPayload_15(v);
    }

    // ---- Closesthit getters (read values passed from raygen) --------------------

    /** @brief Reads bounce index from p17 (set by raygen before trace). */
    __forceinline__ __device__ uint32_t payload_get_bounce() {
        return optixGetPayload_17();
    }

    /** @brief Reads sample_index from p15 (set by raygen before trace). */

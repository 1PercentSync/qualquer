...
namespace qualquer::app {
    ...
    PreparedTexture compress_texture_bc6h(
        ...
        const uint32_t level_count = static_cast<uint32_t>(
            std::floor(std::log2(static_cast<double>(face_size)))) + 1u;

        // Compress each face at each mip level with BC6H. Mip levels beyond
        // the base are generated by downsampling each face independently via
        // stbir's half-float resize path.
        auto compress_cubemap_level = [](const uint16_t *data, const uint32_t size) {
            const uint32_t bpf = block_count(size) * block_count(size);
            const auto face_els = static_cast<uint64_t>(size) * size * 4u;
            std::vector<uint8_t> level_data(static_cast<size_t>(bpf) * 16u * 6u);
            for (uint32_t f = 0; f < 6; ++f) {
                auto face_bc6h = compress_bc6h(data + f * face_els, size, size);
                const size_t dst = static_cast<size_t>(f) * bpf * 16u;
                std::memcpy(level_data.data() + dst, face_bc6h.data(), face_bc6h.size());
            }
            return level_data;
        };

        std::vector<std::vector<uint8_t>> levels(level_count);

        // Level 0: compress directly from input.
        levels[0] = compress_cubemap_level(rgba.data(), face_size);

        // Levels 1+: downsample previous level, then compress.
        // Two alternating buffers avoid reading from and writing to the same
        // allocation (level 0 source is the input span, not a buffer).
        std::vector<uint16_t> src_buf;
        std::vector<uint16_t> dst_buf;
        const uint16_t *src_data = rgba.data();
        uint32_t src_size = face_size;

        for (uint32_t level = 1; level < level_count; ++level) {
            const uint32_t dst_size = std::max(1u, src_size / 2u);
            const auto src_face_els = static_cast<uint64_t>(src_size) * src_size * 4u;
            const auto dst_face_els = static_cast<uint64_t>(dst_size) * dst_size * 4u;
            dst_buf.resize(dst_face_els * 6u);

            for (uint32_t f = 0; f < 6; ++f) {
                stbir_resize(
                    src_data + f * src_face_els,
                    static_cast<int>(src_size), static_cast<int>(src_size), 0,
                    dst_buf.data() + f * dst_face_els,
                    static_cast<int>(dst_size), static_cast<int>(dst_size), 0,
                    STBIR_RGBA, STBIR_TYPE_HALF_FLOAT,
                    STBIR_EDGE_CLAMP, STBIR_FILTER_DEFAULT);
            }

            levels[level] = compress_cubemap_level(dst_buf.data(), dst_size);

            src_buf = std::move(dst_buf);
            src_data = src_buf.data();
            src_size = dst_size;
        }

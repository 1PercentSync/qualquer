...
CLion Nova 引擎只对被 `.cu` 文件 include 的 `.cuh` 赋予 CUDA 编译上下文。新建 `.cuh` 后必须在 `programs.cu` 中添加对应 `#include`，否则 Nova 无法解析 CUDA 内建符号（`sqrtf`、`__int_as_float` 等），产生 `Cannot resolve symbol` 警告。

...
- **设计评估义务**：陈述设计选择时，每条理由必须区分"依据"（引用具体文档或代码位置）和"判断"（基于事实的推断，标注为判断）。不得为任何设计选择编造项目中不存在的约束或需求作为理由。发现任务清单的设计可能有局限时，主动提出而非回避
...
CLion Nova 引擎只对被 `.cu` 文件 include 的 `.cuh` 赋予 CUDA 编译上下文。新建 `.cuh` 后必须在 `programs.cu` 中添加对应 `#include`，否则 Nova 无法解析 CUDA 内建符号，产生 `Cannot resolve symbol` 警告。
...
`git checkout -- <file>` 会无差别丢弃该文件**全部**未提交内容，用于“回退局部某行”时属越权。如需精准回退，用 patch 模式或手动编辑只移除真正要撤的几行。
...
每个资源只有一个所有者（owner）。其他对象作为使用方，在 init/destroy/recreate 等操作时从调用方接收所有者的引用，**不**在自身成员中缓存任何由所有者拥有的句柄或引用。

...
- `renderer/include/qualquer/renderer/nee.cuh:600-639` 先分别生成环境光与发光三角形候选，再以本次候选的 `env_lum` 和 `emi_lum` 计算 `p_env`。
- `renderer/include/qualquer/renderer/nee.cuh:655-670` 在环境光 NEE 侧使用 `p_env * env_pdf` 参与 MIS。
- `renderer/include/qualquer/renderer/nee.cuh:679-700` 在发光体 NEE 侧使用 `(1-p_env) * emissive_pdf` 参与 MIS。
- `renderer/src/device/programs.cu:418` 的 BRDF miss 路径使用场景级选择概率 `p`。
- `renderer/src/device/programs.cu:595` 的 BRDF emissive-hit 路径也使用场景级选择概率 `p`。

#### 判断

NEE 侧的选择概率由“本次随机生成的两个候选 radiance”决定；BRDF-hit 侧无法重建这两个未被 BRDF 路径采样的随机候选，改用场景总量概率。因而同一光传输路径的两个竞争策略没有使用相同的 mixture PDF，power heuristic 权重不再互补。

将 NEE 贡献除以本次 `p_env` 或 `1-p_env` 只能保证不含 MIS 时的随机策略选择无偏，不能修复 NEE 与 BRDF-hit 使用不同竞争 PDF 所造成的 MIS 偏差。

场景级概率还比较了不同量纲/归一化方式的总量：环境 alias 总量来自离散 cell 的 `luminance × sin(theta)`，发光体总量来自 `luminance × world-space area`。环境 alias 分辨率变化也会改变前者数值。

#### 触发条件

环境光与发光三角形同时存在，并启用当前单策略 NEE。

#### 影响

- 结果可能产生系统性亮度偏差，而不仅是方差变化；
- 环境 alias 降采样分辨率可能在物理照明未变化时改变 BRDF-hit MIS 权重；
- 画面偏差会随候选 radiance、场景面积和 alias 分辨率变化，难以通过单一场景目测排除。
...
shadow terminator 附近产生有方向/PDF依赖的能量偏差；改变 roughness、light sampling 分布或 QRP-003 的 strategy selection 甚至可能改变该偏差，而不只是噪声。BRDF-hit 与纯间接分支还会重新引入本想消除的亮边/漏光。
...
2. **再统一 light transport estimator**：QRP-003、QRP-006、QRP-008～010、QRP-020、QRP-032、QRP-033、QRP-039。这里包含 MIS mixture、几何/着色法线域和 alpha-emissive domain 三组相互影响的问题，必须一起做无偏 reference 验证。
...
| 光输运 reference | 003、006、008～010、018、020、032、033、039、042 | 关闭 firefly clamp，用 CPU/reference path tracer 或高样本 ground truth 对比；分别测纯 env、纯 emissive、混合光源、纬度高亮 env、负 scale、single-sided shell、强 normal map、alpha-masked emissive 与 roughness 0。记录均值偏差与 RMSE，不只看降噪输出。 |
